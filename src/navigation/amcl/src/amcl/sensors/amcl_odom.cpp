...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  break;
  case ODOM_MODEL_DIFF_SIGNED:
  {
    // Keep the stock differential model for normal motion.  For motion below
    // its 1 cm bearing guard, preserve the sign of longitudinal travel.  The
    // stock model keeps delta_trans positive and therefore mirrors every
    // short reverse update into forward motion.
    double delta_rot1, delta_trans, delta_rot2;
    double delta_rot1_hat, delta_trans_hat, delta_rot2_hat;
    double delta_rot1_noise, delta_rot2_noise;
    ...
    if(delta_trans < 0.01)
    {
      ...
      const double longitudinal_delta =
          ndata->delta.v[0] * cos(old_pose.v[2]) +
          ndata->delta.v[1] * sin(old_pose.v[2]);
      // Keep the near-zero atan2 guard and carry reverse direction in signed
      // translation.  This avoids placing noisy rotations on the +/-pi branch
      // cut, whose numerically unwrapped samples can split KLD yaw bins.
    ...
    // Noise depends on squared translation, so signed travel preserves the
    // stock uncertainty magnitude while correcting deterministic direction.
    delta_rot1_noise = std::min(fabs(angle_diff(delta_rot1,0.0)),
                                fabs(angle_diff(delta_rot1,M_PI)));
    delta_rot2_noise = std::min(fabs(angle_diff(delta_rot2,0.0)),
                                fabs(angle_diff(delta_rot2,M_PI)));
