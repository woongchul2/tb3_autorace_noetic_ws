...
void loop()
{
  ...
  if ((t-tTime[0]) >= (1000 / CONTROL_MOTOR_SPEED_FREQUENCY))
  {
    ...
    {
      motor_driver.controlMotor(MOTOR_COMMAND_RADIUS, WHEEL_SEPARATION, zero_velocity);
    } 
    else {
      motor_driver.controlMotor(MOTOR_COMMAND_RADIUS, WHEEL_SEPARATION, goal_velocity);
    }
    tTime[0] = t;
  }
  ...
  // Sonar is not installed on this robot; keep sonar update disabled.
...
void publishSensorStateMsg(void)
{
  ...
  // Sonar is not installed on this robot; leave sensor_state_msg.sonar unused.
...
void updateMotorInfo(int32_t left_tick, int32_t right_tick)
{
  ...
  last_rad[LEFT]       += WHEEL_TICK2RAD * (double)last_diff_tick[LEFT];

  current_tick = right_tick;

  last_diff_tick[RIGHT] = current_tick - last_tick[RIGHT];
  last_tick[RIGHT]      = current_tick;
  last_rad[RIGHT]       += WHEEL_TICK2RAD * (double)last_diff_tick[RIGHT];
}
...
bool calcOdometry(double diff_time)
{
  ...
  wheel_l = WHEEL_TICK2RAD * (double)last_diff_tick[LEFT];
  wheel_r = WHEEL_TICK2RAD * (double)last_diff_tick[RIGHT];
...
void driveTest(uint8_t buttons)
{
  ...
  if (buttons & (1<<0))  
  {
    ...
    diff_encoder = TEST_DISTANCE / DISTANCE_PER_MOTOR_TICK;
    tTime[6] = millis();
  }
  else if (buttons & (1<<1))
  {
    ...
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / DISTANCE_PER_MOTOR_TICK;
    tTime[6] = millis();
  }
